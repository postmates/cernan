...
impl Statsd {
    pub fn new(chans: Vec<mpsc::Sender<metric::Event>>, port: u16) {
        ...
        let socket_v6 = UdpSocket::bind(addr).expect("Unable to bind to UDP socket");
        let addr_v4 = SocketAddrV4::new(Ipv4Addr::new(127, 0, 0, 1), port);
        let socket_v4 = UdpSocket::bind(addr).expect("Unable to bind to UDP socket");
        Statsd {
            chans: chans,
            socket_v4: socket_v4,
            socket_v6: socket_v6,
    ...
    pub fn handle_udp(&mut self, socket: UdpSocket) {
        let mut buf = [0; 8192];
        loop {
            ...
            debug!("[statsd] recv time elapsed (ns): {}", recv_time.elapsed().subsec_nanos());
                ...
                .map(|val| {
                    ...
                    match metric::Metric::parse_statsd(val) {
                        Some(metrics) => {
                            ...
                            debug!("[statsd] payload handle effective, elapsed (ns): {}", pyld_hndl_time.elapsed().subsec_nanos());
                        }
                        None => {
                            ...
                            // NOTE this is wrong! See above NOTE.
                            debug!("[statsd] payload handle failure, elapsed (ns): {}", pyld_hndl_time.elapsed().subsec_nanos());
                        }
                    }
                })
                .ok();
...
impl Source for Statsd {
    fn run(&mut self) {
        ...
        joins.push(thread::spawn(move || {
            handle_udp(self.socket_v4)
        }));
        joins.push(thread::spawn(move || {
            handle_udp(self.socket_v6)
        }));
