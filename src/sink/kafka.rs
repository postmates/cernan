...
impl Sink<KafkaConfig> for Kafka {
    ...
    /// Fire off the given event to librdkafka. That library handles buffering
...
impl Kafka {
    ...
    /// Wait on all in-flight messages, and return an `OwnedMessage` for each
    /// message that needs to be retried.
